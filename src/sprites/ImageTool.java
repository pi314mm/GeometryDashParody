...
public abstract class ImageTool{
	 ...
	 */
	public static Image readImage(InputStream imageFile){
		BufferedImage bimg=null;
		try{
			bimg = ImageIO.read(imageFile);
		}catch(Exception e){
			System.err.println("Error: Image file "+imageFile+" is invalid");
			e.printStackTrace();
		}
		return toImage(bimg);
	}
	...
	/**
	 * Recolors a black and white image to a gradient of the color given
	 * 
	 * @param img
	 *            Image to color
	 * @param color
	 *            darkest color of gradient
	 * @return colored image
	 */
	public static Image recolor(Image img, Color color){
		BufferedImage bimg = toBufferedImage(getEmptyImage(img.getWidth(null),img.getHeight(null)));
		Graphics2D g = bimg.createGraphics();
		g.drawImage(img,0,0,null);
		g.dispose();
		double[] modifier = {color.getRed() / 255.0, color.getGreen() / 255.0, color.getBlue() / 255.0};
		for(int row = 0; row < bimg.getHeight(); row++){
			for(int col = 0; col < bimg.getWidth(); col++){
				Color pixel = new Color(bimg.getRGB(col,row),true);
				bimg.setRGB(col,row,new Color((int)(pixel.getRed() * modifier[0]),(int)(pixel.getGreen() * modifier[1]),(int)(pixel.getBlue() * modifier[2]),bimg.getTransparency()).getRGB());
			}
		}
		return toImage(bimg);
	}
